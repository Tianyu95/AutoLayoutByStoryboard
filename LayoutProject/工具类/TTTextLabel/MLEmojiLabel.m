...
//    [self initPlist];
    ...
    UIColor *commonLinkColor = [UIColor colorWithRed:69/255.0 green:75/255.0 blue:165/255.0 alpha:1];
...
{
    if (result.resultType == NSTextCheckingTypeCorrection) {//这里是超链接等点击
        //判断消息类型
        ...
        if ( result != nil ) {//这里是指定文字点击
            
            if(self.emojiDelegate&&[self.emojiDelegate respondsToSelector:@selector(mlEmojiLabel:didSelectLink:withType:)]){
                //type的数组和i刚好对应
                [self.emojiDelegate mlEmojiLabel:self didSelectLink:[self.text substringWithRange:result.range] withType:MLEmojiLabelLinkTypeAppointedcharacters];
            }
            
        }else{//这里是除超链接、指定文字外部分的点击
            
            if(self.emojiDelegate&&[self.emojiDelegate respondsToSelector:@selector(mlEmojiLabel:didSelectLink:withType:)]){
                //type的数组和i刚好对应
                [self.emojiDelegate mlEmojiLabel:self didSelectLink:[self.text substringWithRange:result.range] withType:MLEmojiLabelLinkTypeExpectForAppointRegion];
